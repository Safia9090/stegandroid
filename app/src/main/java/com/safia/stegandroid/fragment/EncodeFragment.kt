package com.safia.stegandroid.fragment
...
import com.safia.stegandroid.R
...
class EncodeFragment : Fragment(), TextEncodingCallback, View.OnClickListener {
                ...
                button_save.setOnClickListener {
                    if (encoded_image != null && original_image != null) {
                        if (edittext_key.text != null && edittext_message != null) {
                            ...
                            Toast.makeText(activity, "Isi key dan message", Toast.LENGTH_SHORT)
                                .show()
                        }
                    } else {
                        Toast.makeText(activity, "Pilih dan Encode gambar", Toast.LENGTH_SHORT)
                            .show()
                    }
                }
            }
    ...
    private fun saveToInternalStorage(bitmapImage: Bitmap?) {
        ...
        bitmapImage?.compress(
            Bitmap.CompressFormat.PNG, 10, fOut
        ...
        if (!listPermissionsNeeded.isEmpty()) {
            activity?.let {
                ActivityCompat.requestPermissions(
                    it, listPermissionsNeeded.toTypedArray(), 1
                )
            }
        }
    }
